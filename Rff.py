...
"""%s
 ...
 - author      : Rosyid Gans
 - facebook    : https://www.facebook.com/paitonguchuu.uchuu
 - fanspage    : facebook.com/100004986947878
 - whatsap     : YHAAAAA RAHASIA
 - github      : github.com/ROSYIDXD
 - script name : bff-2
 - version     : 1.2
...
def dumpfl():
    ...
    if len(bs4.re.findall('logout', r)) != 0:
        ...
        print ("%s%s%s Example nama orang %s[ %sRosyid Ganteng %s]"%(U,til,O,P,H,P))
        s=raw_input("%s%s%s Sett nama %s> %s"%(U,til,O,M,K))
        if s in("rosyid","Rosyid","ROSYID","Rosyid Gans","Rosyid gans","ROSYID GANS","rosyid gans"):
        	print("\n%s%s anak anjing mau crack pake nama gw "%(M,til));exit()
        elif s in("Rosyid Ganteng","Rosyid ganteng","ROSYID GANTENG","rosyid ganteng"):
        	print ("\n%s%s memang ganteng dong abang Rosyid"%(H,til));exit()
...
def opsi():
	...
	rosyid = raw_input("%s%s%s Nama file %s> %s"%(U,til,O,M,K))
	if rosyid == "":
		print("%s%s isi yang benar "%(M,til));jeda(2);opsi()
	try:
		file_cp = open(hasil+rosyid, "r").readlines()
	except IOError:
		exit("\n%s%s nama file %s tidak tersedia"%(M,til,rosyid))
